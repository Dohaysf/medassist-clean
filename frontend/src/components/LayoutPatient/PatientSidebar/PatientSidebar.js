...
import translations from '../../../translation'; // ✅ Chemin corrigé
...
const PatientSidebar = () => {
    ...
    const t = (translations[lang]?.sidebar) || translations.fr.sidebar;
    const isRTL = lang === 'ar';
    ...
    const [theme, setTheme] = useState(() => {
        try {
            const saved = JSON.parse(localStorage.getItem('medassist_settings'));
    ...
    // ======================== RENDU ===========================
    return (
        <aside className={`Patient-sidebar ${collapsed ? 'collapsed' : ''} ${isRTL ? 'rtl' : ''}`} dir={isRTL ? 'rtl' : 'ltr'}>
            {/* Header */}
            ...
            <button className="new-chat-btn" onClick={newConsultation}>
                ...
                {!collapsed && <span>{t.newConsultation || 'Nouvelle consultation'}</span>}
            </button>
                    ...
                    <div className="sidebar-search-wrapper" style={{ padding: '0 6px', marginBottom: '8px' }}>
                        {showSearch ? (
                            <div className="sidebar-search-active" style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '8px 12px', background: 'rgba(0,0,0,0.06)', borderRadius: '8px' }}>
                                ...
                                />
                                <button className="search-close-btn" onClick={() => { setShowSearch(false); setSearchQuery(''); }} style={{ background: 'none', border: 'none', cursor: 'pointer' }}>
                                    <IconClose />
                                </button>
                            </div>
                        ) : (
                    ...
                    <div className="sidebar-recents">
                        ...
                        {filteredSessions.length === 0 ? (
                            <div className="recent-item-empty">
                                {searchQuery ? (t.noResults || 'Aucun résultat') : (t.noRecent || 'Aucune discussion récente')}
                            </div>
                        ) : (
            ...
            <div className="Patient-sidebar-footer">
                ...
                <button onClick={handleLogout} className="Patient-logout-btn">
                    ...
                    {!collapsed && <span>{t.logout || 'Déconnexion'}</span>}
                </button>
            </div>
        </aside>
    );
